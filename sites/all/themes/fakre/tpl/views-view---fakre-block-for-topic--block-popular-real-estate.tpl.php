<?php
	...
	$query = db_select('node', 't')
			...
			->condition('type', 'real_estate');

	$query->join('node_counter', 'n', 'n.nid = t.nid');
		...
		<div class="col-xs-12">
			...
			<nav class="isotop-controls4 no-bg margin-bottom-30">
				...
				<?php foreach ($allowed_values as $key => $value) {
					...
					$slide = '<div id="'.$key.$i.'"><div class="beans-slider">';
					...
					if ($i == 0) {
						...
						print '<li class="active"><a href="#'.$key.$i.'">'.$allowed_values[$key].'</a></li>';

					} else {

						print '<li><a href="#'.$key.$i.'">'.$allowed_values[$key].'</a></li>';
